    ...
    else if (requestType==RequestTypeMultiUpload)
    {
        [session POST:url parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
            ...
            for (NSInteger i = 0; i < dataArray.count; i++)
            {
                ...
                //name和服务端约定好
                [formData appendPartWithFileData:imageData name:[NSString stringWithFormat:@"file%zi",i+1] fileName:[NSString stringWithFormat:@"%zi.jpg",i] mimeType:@"image/jpeg"];
            }
        }
            progress:^(NSProgress * _Nonnull uploadProgress) {
            //(float)uploadProgress.completedUnitCount/(float)uploadProgress.totalUnitCount
...
//        //根据返回的接口内容来变
        id success = [response objectForKey:@"status"];
        NSString* message = [response objectForKey:@"msg"];
        //success存在,并且为0时
        if([success isEqualToString:@"failure"])//处理掉0的情况,直接进报错处理
        {
